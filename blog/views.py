...
def post_detail(request, year, month, day, post):
    ...
    # QuerySet, starting from the post object.
    ...
    # Returns tuples with the values for the given fields.
    post_tags_ids = post.tags.values_list('id', flat=True)
    # Get all posts that contain any of these tags, excluding the current post itself.
    similar_posts = Post.published.filter(tags__in=post_tags_ids) \
        .exclude(id=post.id)
    # Count aggregation function to generate a calculated field
...
def post_share(request, post_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            send_mail(subject, message, 'YOUR_MAIL@mail.com', [cd['to']])
            sent = True
